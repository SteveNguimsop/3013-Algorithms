...
void printMatches(json d, string sub) {
  for (auto &element : d.items()) {
    ...
    if (key.find(sub) != string::npos) {
      ...
      cout << "Found partial match: " << key << " -> " << val << endl;
    }
  }
}
// new addition
...
vector<string> partialMatch(json array, string substring) {
  ...
  for (auto &element : array.items()) {
    ...
    found = key.find(substring);               // check for substr match
...
int main() {
  ...
  /********** New addtion *******/
  json dict = loadJsonFile("./data/dictionary_small_2.json");
  ...
  while ((k = getch()) != 'Z') {
    ...
    // our substr word
    matches = partialMatch(dict, substr);
    ...
    if ((int)k != 32) { // if k is not a space print it
      ...
      if (matches.size() == 1) {
        cout << "You found a match! " << matches[0] << endl;
        cout << "done?" << endl;
